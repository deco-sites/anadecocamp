    ...
    <div
      class="container py-8">  
        ...
        <div class="flex flex-wrap max-w-[960px] mx-auto">
          {imagens.slice(current, 2).map((item, index) => (
          ...
          <div class="w-full">
            <button class="btn mt-4 py-1 px-8 block bg-primary w-auto mx-auto text-white border-0 h-auto min-h-0 md:text-sm" {...usePartialSection({ props: {imagens: imagens.slice(0, 4)} })}>Ver Mais</button>
          </div>
        </div>
    </div>
  );
